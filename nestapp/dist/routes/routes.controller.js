...
const router_serializer_1 = require("./router.serializer");
let RoutesController = class RoutesController {
    ...
    async create(createRouteDto) {
        ...
        return new router_serializer_1.RouteSerializer(route);
    }
    async findAll() {
        const routes = await this.routesService.findAll();
        return routes.map((route) => new router_serializer_1.RouteSerializer(route));
    }
    async findOne(id) {
